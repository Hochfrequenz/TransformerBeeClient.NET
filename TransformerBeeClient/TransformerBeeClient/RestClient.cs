...
public class TransformerBeeRestClient : ICanConvertToBo4e, ICanConvertToEdifact
{
    ...
    public TransformerBeeRestClient(
        IHttpClientFactory httpClientFactory,
        ITransformerBeeAuthenticator authenticator,
        string httpClientName = "TransformerBee"
    )
    {
    ...
    public async Task<List<Marktnachricht>> ConvertToBo4e(
        string edifact,
    ...
    )
    {
        ...
        var request = new EdifactToBo4eRequest { Edifact = edifact, FormatVersion = formatVersion };
        ...
        if (!httpResponse.IsSuccessStatusCode)
        {
            ...
            if (
                ...
                && !_authenticator.UseAuthentication()
            )
            {
                throw new AuthenticationException(
                    $"Did you correctly set up the {nameof(ITransformerBeeAuthenticator)}?"
                );
            }

            throw new HttpRequestException(
                $"Could not convert {edifact} to BO4E. Status code: {httpResponse.StatusCode} / {errorContent}"
            );
        }

        var responseContent = await httpResponse.Content.ReadAsStringAsync();
        var bo4eResponse = JsonSerializer.Deserialize<EdifactToBo4eResponse>(
            responseContent,
        ...
        var result = JsonSerializer.Deserialize<List<Marktnachricht>>(
            unescapedJson!,
            _jsonSerializerOptions
        );
    ...
    public async Task<string> ConvertToEdifact(
        BOneyComb boneyComb,
        EdifactFormatVersion formatVersion
    )
    {
        ...
        if (!httpResponse.IsSuccessStatusCode)
        {
            ...
            throw new HttpRequestException(
                $"Could not convert to EDIFACT; Status code: {httpResponse.StatusCode} / {errorContent}"
            );
        }

        var responseContent = await httpResponse.Content.ReadAsStringAsync();
        // todo: ensure that the deserialization does not fail and the response is not empty
        var responseBody = JsonSerializer.Deserialize<Bo4eTransactionToEdifactResponse>(
            responseContent!,
            _jsonSerializerOptions
        );
